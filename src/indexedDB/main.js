...
function store(cidade) {
    ...
    transaction.onerror = function (event) {
        console.log(
            "Transação  readwrite em store() finalizada com erro. Erro: " +
            event.target.error
        );
        event.preventDefault();
    };
...
function deletar(id) {
    ...
    transaction.onerror = function (event) {
        console.log(
            "Transação  readwrite em deletar() finalizada com erro. Erro: " +
            event.target.error
        );
    };
...
function getCidade(id) {
    ...
    transaction.onerror = function (event) {
        console.log(
            "Transação  readonly em get() finalizada com erro. Erro: " +
            event.target.error
        );
    };
...
function getDetalhes(id) {
    ...
    request.onsuccess = function (event) {
        ...
        let h1 = $("<h1>").text(
            event.target.result.nome + " / " + event.target.result.estado
        );
        let ibge = $("<h3>").text("Código do IBGE: " + event.target.result.ibgeCod);
        let pop = $("<h3>").text(
            "População estimada: " + event.target.result.populacao
        );
        let obs = $("<p>");
...
function listar() {
    ...
    transaction.onerror = function (event) {
        console.log(
            "Transação  readonly em listar() finalizada com erro. Erro: " +
            event.target.error
        );
    };
...
function popularLista() {
    ...
    cidades.forEach((element) => {
        ...
        let infos = [
            $("<div>")
            .addClass("info")
            .append(element.nome + " / " + element.estado),
            $("<div>")
            .addClass("info")
            .append("População estimada: " + element.populacao),
            $("<div>")
            .addClass("info")
            .append("Código IBGE: " + element.ibgeCod),
        ];
        item.append(infos);
        let divButton = $("<div>").addClass("mais");
        divButton.append(
            $("<button>")
            .addClass("btn detalhes")
            .append("Ver mais")
            .attr("id", element.id)
        );
        let divMenu = $("<div>").addClass("menu");
        divMenu.append(
            $("<div>").addClass("editar item-icon").attr({
                id: element.id,
            })
        );
        divMenu.append(
            $("<div>").addClass("deletar item-icon").attr({
                id: element.id,
            })
        );
